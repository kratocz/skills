...
This skill executes a code review following the user's code-review conventions. If the project or global instructions file (`AGENTS.md` / `CLAUDE.md`) has a `## Code review` section (severity codes, focus areas, comment language, review process), read it first; this skill is the **procedure** to follow.
...
2. **Gather inputs and check for prior CR rounds.** Read everything relevant: all commits on the target, the PR description, all existing review and discussion comments (including author replies), and any prior findings files in `docs.local/code-reviews/` matching `cr-pr-<number>-*.md` (PR targets) or `cr-<slug>-*.md` (branches/other targets).
   - **If the PR is already merged, the review is a different job — say so up front and adjust.** A post-merge pass is not a gate; its value is telling the reader what is *still true of `main` today*. Establish three states separately and never conflate them: the PR as merged, `origin/main` now, and the local checkout (which in a worktree is often behind both). Then, for every finding, check whether a later commit already fixed it — a follow-up ticket may have landed between the merge and your review, and re-raising a closed gap wastes the author's time and undermines the rest of the review. Say explicitly which findings are live on `main` and which are historical. Weight effort toward what someone could still act on.
   - No prior CR → proceed.
   ...
   - **Infrastructure PRs (Terraform/OpenTofu, Kubernetes manifests): run the plan on a throwaway local merge of the integration branch into the PR head, never on the PR head alone.** A PR branch is usually behind `main`, so a plan from its head "reverts" everything applied since the branch point and buries the PR's own changes in phantom noise (2026-09-24, PR #247: the raw-head plan removed CORS origins that were simply not on the branch yet). `git checkout -b cr-<N>-merge-check pr-<N> && git merge origin/main`, plan there, and compare the resource list against what the PR claims to add; delete the branch when the review is posted.
...
4. **Produce findings.** Label each one with a severity code (`C1`, `C2`, `M1`, `m1`, `n1`, …). If a finding needs a new category (e.g. off-topic), propose it to the user with a suggested letter prefix.
   ...
   - **Worktree-isolated session:** write the findings file into the **worktree's** `docs.local/code-reviews/` (create it; the `.gitignore` entry already covers it), since that is always reachable. Prior rounds may still live in the main checkout's `docs.local/code-reviews/` — read them from there. Then, because the worktree's copy vanishes with the worktree, **copy the file into the main checkout yourself** at the end of the CR: a plain `cp` with **absolute paths on both sides** went through from inside a worktree on 2026-09-07, so do not assume it is blocked. Earlier revisions of this skill stated flatly that such a write "gets blocked by the isolation guard" and told you to hand the command to the user; that overstated it — the guard is about git operations and about paths it cannot statically verify, not about every write outside the worktree. If the `cp` *is* refused, then offer it to the user to run, still with absolute paths on both sides: a relative path assumes their shell sits at the repo root, and a worktree-relative `cp .claude/worktrees/<name>/docs.local/… docs.local/` fails with `No such file or directory`, which reads as if the findings file were missing when only the command was wrong.
...
11. **Third verification pass:** final check of everything to avoid wasting the author's time on inaccuracies.
    - **Then an adversarial refutation pass in a fresh context.** Dispatch a subagent with ONLY the findings file and the diff (not your reasoning or verdict), briefed to *refute* each finding and to look for what the review missed. A reviewer handed conclusions returns validation of those conclusions; a refuter finds the gaps. Record the outcome per finding in the findings file (stands / downgraded / refuted / new). On 2026-09-24 (PR #247) this pass produced the review's only staging-visible finding — the three earlier passes had walked past it.
...
14. **After approval, post to GitHub** (skip this step if the target is not a GitHub PR — the findings file is then the only deliverable):
    ...
    - **Partial-scope review → `Comment`, not `Approve`.** When the user asked for only part of the PR (e.g. "just the infra part"), open the summary with the scope you actually read and say what you did not; submit the review as a COMMENT even with zero blockers, because a green Approve reads as a sign-off on the whole PR. Offer Approve only if the user explicitly wants the scoped verdict to carry that weight.
