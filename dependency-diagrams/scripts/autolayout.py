...
Takes a graph (nodes + edges as JSON, e.g. the output of gen_diagrams.py),
runs `dot` to position the nodes, and emits a .drawio file with the
...
# When a grouped graph is laid out, each top-level group takes the next colour
# (cycled in a fixed, harmonious order) so related modules read as a coloured
...
def build_dot(graph):
    ...
    lines = [f"digraph G {{ rankdir={rankdir}; splines=ortho; nodesep=0.45; ranksep=1.6; node [shape=box fixedsize=true];"]
...
def to_drawio(graph, height, pos, edge_pts, color=True):
    ...
    for i, edge in enumerate(graph.get("edges", [])):
        ...
        estyle = edge.get("style", EDGE_STYLE)   # per-edge style wins over default
        cells.append(
